...
public interface ChangeDetailRepository extends PagingAndSortingRepository<ChangeDetail, Integer> {
	@Modifying
	@Query(value = "insert into change_detail(id_order_detail,id_change,imaysp) values(?1," +
			"(select id_change from product_change order by id_change  desc limit 1),?2)",nativeQuery = true)
	void createChangeDetails(Integer id, String imaysp);
